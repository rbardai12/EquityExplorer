...
rem 3. Build the Java binary path using the JAVA_HOME value, validate that 'JAVA_HOME\bin\java -fullversion' is the valid Java supported, which is Java8, Java11 and OpenJDK8.

rem Provide the absolute path to Java8, Java11 or OpenJDK8 installation (optional)
rem NOTE: If the customer requires Java_Home to be set to < 1.8, then override this
rem section with the directory for Java8, Java11 or OpenJDK8)
set JAVA=
...
if [!JAVA!] equ [] (
    echo Absolute path to JAVA 8 or Java 11 or OpenJDK8 executable not provided. Will attempt to figure out path from JAVA_HOME or Windows registry...
...
:OLD_JAVA_ERROR
echo ERROR: The Java version you are using is not supported by Anaplan Connect. Please upgrade to JAVA 8 or Java 11 or OpenJDK8.
exit

:BAD_JAVA_ERROR
echo ERROR: The Java version with major.minor (%1.%2) Java is not supported by Anaplan Connect. Only supported versions are JAVA 8 or Java 11 or OpenJDK8.
exit
...
rem Test for Java version (only JAVA 8 or Java 11 or OpenJDK8 supported), not Java 6 or 7
...
if !current_version_major! geq !bad_version_major! (
    if !current_version_major! equ 9 (
    	 call :BAD_JAVA_ERROR !current_version_major! !current_version_minor!
    )
    if !current_version_minor! equ 0 (
	 echo Current Java version is valid for Anaplan Connect.
	 goto :eof
	)
	if !current_version_minor! equ 8 (
	 echo Current Java version is valid for Anaplan Connect.
	 goto :eof
	)
	if !current_version_minor! neq 8 (
	 call :BAD_JAVA_ERROR !current_version_major! !current_version_minor!
	)
) else (
    call :BAD_JAVA_ERROR !current_version_major! !current_version_minor!
...
rem Java version is newer than 1.8 so we can continue running
rem Set up the classpath
set HERE=%~dp0
set LIB=%HERE%lib
rem Pick up the most recent Anaplan Connect Jar
set CP=
for /f %%l in ('dir /b /od "%HERE%\anaplan-connect-*-jar-with-dependencies.jar"') do set CP=%HERE%%%l
